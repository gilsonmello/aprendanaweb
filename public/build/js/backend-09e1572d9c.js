...
$(function(){
    ...
    $(".zip").on('blur', function (e) {
        findAddressBrazil($("#zip"), $("#city_id"), $("#city"), $("#state"), $("#neighborhood"), $("#address"));
    });
...
function add_course_teachers(){
    ...
    $("[name |='course-teacher-name']").each(function(index){
        ...
        var teacher_percentage = $(this).parent().parent().find("[name|='course-teacher-percentage']").val();
...
function createModuleTable(){
    ...
    $("[name|='module-name']").on('keypress',function(e){
        if(e.which == 13) {
            $(this).next('.add-module').click();
        }

    });

    $("[name|='lesson-name']").on('keypress',function(e){
        if(e.which == 13) {
    ...
    $('#module-table tbody').on( 'click', 'tr', function () {
        if ( $(this).hasClass('selected') ) {
            ...
            $('<li><a href="#edit-module-'+ table.row( this ).data()[0] + '" class="dynamic-tab" data-toggle="tab">Edição do Módulo '+table.row(this).data()[1]+'</a></li>').appendTo('.nav-tabs');
    ...
    $("#lessons-table").on( 'click', 'tr', function () {
        ...
        if ( $(this).hasClass('selected') ) {
            ...
            $('<li><a id="edit-lesson-tab" href="#edit-lesson-'+ selectedLesson + '" class="dynamic-tab" data-toggle="lesson-edit">Edição da Aula '+lesson_name+'</a></li>').appendTo('.nav-tabs');
            $.ajax({
                type: "POST",
                url: '/admin/courses/editlessons',
                data: { lesson_id: selectedLesson
                },
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                },
                cache: false,
                success: function(data){
                    var $appended = $('<div class="tab-pane" id="edit-lesson-'+ selectedLesson + '">' + data + '</div>').appendTo('.tab-content');

                    remove_changed_attribute($appended.attr('id'));
                    change_watcher($appended.attr('id'));

                    $appended.find("#save-lesson").off('click').on('click',function(){
            ...
            $('<li><a id="video-tab" href="#add-videos-'+ selectedLesson + '" class="dynamic-tab" data-toggle="tab">Vídeos de '+lesson_name+'</a></li>').appendTo('.nav-tabs');
            $.ajax({
                type: "POST",
                url: '/admin/courses/editvideos',
                data: { lesson_id: tableLessons.row(this).data()[0]
                },
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                },
                cache: false,
                success: function(data){
                   var $appended_result = $('<div class="tab-pane" id="add-videos-'+ selectedLesson + '">' + data + '</div>').appendTo('.tab-content');


                    remove_changed_attribute($appended_result.attr("id"));
                    change_watcher($appended_result.attr("id"));
                    save_videos(selectedLesson, $appended_result.find("#save-videos"));
    ...
    $('.add-lesson').click(function(e) {
        ...
        var name_input = $(this).parent().find("[name|='lesson-name']").val();
        var sequence_input = $(this).parent().find("[name|='lesson-sequence']").val();
        var duration_input =  $(this).parent().find("[name|='lesson-name']").val();
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                var new_div = parent_div.clone();

                clear_form_elements(new_div);

                var last_div = $(".lesson-box [name='" + new_div.attr('name') + "']").last();
                console.log("[name='" + new_div.attr('name') + "']");
                console.log(last_div.find("[name='lesson-sequence']"));
...
function add_material(selectedLesson, lesson_name){
    $('<li><a id="material-tab" href="#add-material-'+ selectedLesson + '" class="dynamic-tab" data-toggle="tab">Materiais de '+lesson_name+'</a></li>').appendTo('.nav-tabs');
    $.ajax({
        type: "POST",
        url: '/admin/courses/material',
        data: {
        ...
        cache: false,
        success: function(data){
            var $appended_material = $('<div class="tab-pane" id="add-material-'+ selectedLesson + '">' + data + '</div>').appendTo('.tab-content');

            if($appended_material.length > 0)
            remove_material();
            ...
            /*
            $("[name='material-file']").on('click',function(){
                $("#material").val($(this).text());
                $("#material-link").html($(this).html());
                $("#material-link").attr('href',$(this).data('url'));
                $("[name='material-id']").val($(this).data('id'));
                $(this).css('text-shadow: 1px 1px 0 #27ae60');
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result["extension"] == '.pdf') {
                    ...
                    li = li + '<a name="material-file"  data-id="'+ result['id']+'" data-url="'+result['url']+'" href="/'+ result['url'] +'">'+ result['name'] + '</a> <a type="button" class="fa fa-times remove-material" data-content="'+ result['id'] +'" style="color:red"></a></li>';


                $("#content-files").append(li);
...
function add_teachers(selectedLesson, lesson_name){
    ...
    $('<li><a id="teacher-percentage-tab" href="#add-teachers-'+ selectedLesson + '" class="dynamic-tab" data-toggle="tab">Professores de '+lesson_name+'</a></li>').appendTo('.nav-tabs');
    console.log('should enter');
    disable_lesson_related_tabs();

    $.ajax({
        type: "POST",
        url: '/admin/courses/editteachers',
        data: { lesson_id: selectedLesson
        },
        headers: {
            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
        ...
        success: function(data){
            ...
            var $appended_teachers = $('<div class="tab-pane" id="add-teachers-'+ selectedLesson + '">' + data + '</div>').appendTo('.tab-content');
            enable_lesson_related_tabs(selectedLesson);
            //$(".select2").select2();

            save_teachers(selectedLesson,$appended_teachers.find("#save-teachers"), $appended_teachers.find("[name|=teacher-name]"));
           ...
           var newFormTeachers;
            ...
            $(".add-teacher").off("click").on("click",function(){
                ...
                equal_percentage(sizeDivsTeachers, $appended_teachers.find("[name|='teacher-percentage']"));

            });
            remove_changed_attribute($appended_teachers.attr("id"));
            change_watcher($appended_teachers.attr("id"));
