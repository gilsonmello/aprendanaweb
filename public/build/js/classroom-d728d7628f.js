...
function with_view(callback){
    ...
    if(current_content == $("[name='lesson-content']").data('content')){
        callback();
    }else{
    ...
    with_view(function(){
        ...
        $("[name='lesson-content']").load(function () {
            ...
            player.addEvent('ready', function () {
                ...
                console.log('ready');
                $("#exceeded").html('');
                if ($("#actual-content").data('actual-content') == 12323) {
                    //  $("[name='block-" + $("#actual-content").data('actual-content') + "']").click();
                    // $("#actual-content").data('actual-content', 1);
                } else {
                    console.log('aqui');
                    ...
                    function onPlayProgress(data, id) {
                        if (view_count >= view_max) {
                            player.api('unload');
                            $("#exceeded").html("<i class='fa fa-hand-stop-o' style='color:red'></i>&nbsp;<b>Você excedeu o limite de vizualizações para essa aula</b>");
                        }
                        ...
                        if (progress.indexOf(percentual) == -1 && !isNaN(percentual)) {
...
function goto_last_spot(player){
    ...
    if(progress != null && progress.length > 3) {
        var duration = player.api('getDuration',function(value,player_id)
        {
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            $.each(response, function(i,value){
                ...
                $("#note-panel").append('<li><div> <label class="todo-label" for="todoListItem2"><strong>' + ("0" + minutes).slice(-2) + ':' + ("0" + seconds).slice(-2) + '</strong> | <span>' + value.note + '</span></label> </div> <div class="todo-actions"> <a class="todo-remove exclude-note" data-note-id="'+ value.id +'" href="#"> <i class="fa fa-times"></i> </a> </div> </li>');
...
function load_view_info(){
    ...
    switch_like(view_like);

}

function switch_like(up_down){
    console.log(up_down);
    if(up_down == 1){
        if($("#thumbs-up").hasClass('fa-thumbs-o-up')){
            $("#thumbs-up").removeClass('fa-thumbs-o-up');
            $("#thumbs-up").addClass('fa-thumbs-up');
            $("#thumbs-down").removeClass('fa-thumbs-down');
            $("#thumbs-down").addClass('fa-thumbs-o-down');
        }
    }else if(up_down == -1){
        if($("#thumbs-down").hasClass('fa-thumbs-o-down')) {
            $("#thumbs-down").removeClass('fa-thumbs-o-down');
            $("#thumbs-down").addClass('fa-thumbs-down');
            $("#thumbs-up").removeClass('fa-thumbs-up');
            $("#thumbs-up").addClass('fa-thumbs-o-up');
        }
    }else{
        $("#thumbs-up").removeClass('fa-thumbs-up');
        $("#thumbs-up").addClass('fa-thumbs-o-up');
...
$(".stars").not(".rate-star").hover(function(){
    if(!$(this).hasClass('voted')){
        $(".rate-star").removeClass("fa-star");
        $(".rate-star").addClass("fa-star-o");
    }
});
...
$(".rate-star").click(function(){
    if(!$(this).hasClass('voted')){
        ...
        $(this).closest(".stars").addClass('voted');
        $(this).closest(".stars").css("color", "gold");

        $(this).nextAll().addClass('voted');
        $(this).addClass('voted');
...
$(".button-askTheTeacherClassroom").on('click', function () {
    $.ajax({
        ...
        success: function (data) {
            if (data.trim() === '1'){
                $("#questionAskTheTeacher").val('Dúvida enviada com sucesso');
            }
...
function create_total_chart(selector){
...
function swap_result(){
    $(".current-attempt").click(function(){
        ...
        $("#attempt-wrong").html("&nbsp;&nbsp;" + ($("#last-exam-results").data("questions-count") - $(this).data('attempt-grade')) + " ERROS");
...
function create_dashboard_result_chart(){
    ...
    var values = [
        ...
        {
            value: Number($('#last-exam-results').data("questions-count")) - Number($("#attempt-canvas").data("attempt")),
            color:"#CCCCCC",
            highlight: "#FF5A5E",
            label: "Erros"
        }
...
function create_mini_right_charts(){
        $(".mini-right-chart").each(function(){
            var values = [
                {
...
function create_mini_previous_charts(){
    $(".mini-previous-chart").each(function(){
        var values = [
            {
...
function create_mini_expected_charts(){
    $(".mini-expected-chart").each(function(){
        var values = [
            {
...
if($("#exam-presentation").length > 0 && $("#complete-result").length == 0){
    htmlPreHighlight = $("#exam-table-info").html();
    $(window).on('beforeunload',function(){
        save_current_time();
        save_question_time();
    });
...
function open_explanation_video() {
    ...
    //$("#increaseFontBt").css("display", "none");
    //$("#reduceFontBt").css("display", "none");
    //$("#highlightBtBlue").css("display", "none");
    //$("#highlightBtRed").css("display", "none");
    //$("#highlightBtYellow").css("display", "none");
    //$("#highlightBtClean").css("display", "none");
        ...
        success: function (data) {
         prepare_for_next();
            ...
            if (data[2] != "") {
                ...
                $("#explanation-teacher").html("<b>Professor: </b>" + data[2]);
            } else {
                $("#explanation-teacher").html("");
            }
                        ...
                        if ($("#results").length > 0) {
                            ...
                            if ($("#next").length <= 0) {
                                ...
                                $(".subject-line").click(function () {
                                    /*
                                    $.ajax({
                                        url: "/exam/subject-recommendation",
                                        type: "POST",
                                        data: {'subject': $(this).data('subject-id')},
                                        headers: {
                                            'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                                        },
                                        cache: false,
                                        success: function (data) {
                                            $("#suggestionModal").html(data);

                                            $("#suggestionModal").modal('toggle');
                                        }
                                    });
                                    */
                                    $(this).css('background-color',"#253885");
                                    $(".subject-" + $(this).data("subject-id")).css("display",'none');
                                });
            ...
            if ($("#results").length > 0) {
                ...
                if($("#next").length <= 0) {
                    ...
                    $(".subject-line").click(function(){
                        ...
                        */
                        $(this).css('background-color',"#253885");
                        $(".card-course").not(".subject-" + $(this).data("subject-id")).css("display",'none');
                    });
...
function result_flip_events(){
    ...
    $(".subject-line").click(function(){
        ...
        */
        ...
        $(this).not('.marked').css('background-color',"#253885");
        $(this).not('.marked').addClass('marked');


        $(".p-bj-course").css("opacity",'1.0');
        $(".p-bj-course").not(".subject-" + $(this).data("subject-id")).not('.toggled').css("opacity",'0.3');
        console.log(".subject-" + $(this).data("subject-id"));
    ...
    $(".modal-explanation").click(function(){
        ...
        $.ajax({
            ...
            type: "POST",
            data: {'question': $(this).data('question'), 'enrollment': $(this).data('enrollment')},
            headers: {
                'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
            },
            cache: false,
    ...
    $("#summary").click(function(){
        ...
        $("#result-title").html("<strong>REVISÃO</strong>");
    });
}
...
function cleanHighlight() {
    ...
    $("#exam-table-info").html(htmlPreHighlight);
    if (sel != ""){
