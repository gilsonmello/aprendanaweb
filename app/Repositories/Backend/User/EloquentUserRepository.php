...
class EloquentUserRepository implements UserContract {
	 ...
	 * @return mixed
	 */
	public function getUsersPaginated($per_page, $f_UserController_name, $f_UserController_role, $status = 1, $order_by = 'id', $sort = 'asc') {
		return User::where('status', $status)->where( function($query) use ($f_UserController_name) {
			$query->where('name', 'like', '%' . $f_UserController_name . '%')
				  ->orWhere('email', 'like', '%' . $f_UserController_name . '%');
		})
		->whereHas('roles', function ($subquery) use ($f_UserController_role){
			$subquery->where('name', 'like', '%'.$f_UserController_role.'%');
		})
     ...
     */
    public function getOnlyTeachers() {
     ...
     * @return mixed
     */
    public function selectUsers($term = '', $order_by = 'name', $sort = 'asc') {

        return User::where('name', 'like', $term.'%')->orderBy($order_by, $sort)->get();
    }
	...
	public function getUsersNewsletters() {
		return User::where('is_newsletter_subscriber', '=', true)->orderBy('name', 'asc')->get();
	}
