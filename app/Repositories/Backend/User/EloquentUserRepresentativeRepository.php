...
use Carbon\Carbon;
use App\Services\Access\Traits\UserRepresentativesAttributes;
use App\User;
 ...
 */
class EloquentUserRepresentativeRepository implements UserRepresentativeContract {
	...
	public function getUserRepresentativesPaginated($per_page, $f_UserRepresentativeController_name, $status = 1, $order_by = 'id', $sort = 'asc') {
		return User::representatives()->where( function($query) use ($f_UserRepresentativeController_name) {
			$query->where('name', 'like', '%' . $f_UserRepresentativeController_name . '%')
				->orWhere('email', 'like', '%' . $f_UserRepresentativeController_name . '%');
			})
	...
	public function create($input) {
		...
		try{
			...
			if ($user->save()) {
				$role = (new EloquentRoleRepository())->findOrThrowException(10);
	...
	public function update($id, $input) {
		...
		$this->checkUserRepresentativeByEmail($input, $user);
	...
	public function selectRepresentatives($term = '', $order_by = 'name', $sort = 'asc') {
		...
		return User::representatives()->where('name', 'like', $term.'%')->orderBy($order_by, $sort)->get();
	}
	 ...
	 * @throws GeneralException
	 */
	private function checkUserRepresentativeByEmail($input, $user)
	{
