...
//			if (($input['occupations'][0] != null) && ($input['occupations'][0] != '0') && (isset($input['occupations'][0]))){
//				$user->occupation_id = $input['occupations'][0];
//			} else {
...
//			}
	...
	public function updateProfile($id, $input) {
		$user = $this->findOrThrowException($id);
		$user->name = preg_replace('/\s+/',' ',$input['name']);
		...
		if ($user->canChangeEmail()) {
			...
			if ($user->email != $input['email'])
			{
				...
				if (User::where('email', $input['email'])->first())
					throw new GeneralException("Esse e-mail já está em uso");
		...
		$user->video_quality = $input['video_quality'];
		}
...
//		}
	...
	public function updatePhoto($id, $new_file_name) {
		...
		throw new GeneralException('Houve um problema ao submeter essa foto.');
	}
	...
	public function updateOccupation($id, $input) {
		...
		$user->last_data_update = Carbon::now();
		$user->save();
	}
