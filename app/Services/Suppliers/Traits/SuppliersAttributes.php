...
namespace App\Services\Suppliers\Traits;

use Carbon\Carbon;
use Illuminate\Support\Facades\Config;

/**
 * Class SuppliersAttributes
 * @package App\Services\Suppliers\Traits
 */
trait SuppliersAttributes {
    ...
    public function getEditButtonAttribute() {
        if ((!\Auth::user()->is("Administrador") && $this->status != 1) || \Auth::user()->is("Administrador"))
            return '<a href="' . route('admin.suppliers.edit', $this->id) . '" class="btn btn-xs btn-primary"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" title="' . trans('crud.edit_button') . '"></i></a>';
    }
    ...
    public function getDeleteButtonAttribute() {
        if ((!\Auth::user()->is("Administrador") && $this->status != 1) || \Auth::user()->is("Administrador"))
            return '<a href="' . route('admin.suppliers.destroy', $this->id) . '" data-method="delete" class="btn btn-xs btn-danger"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="' . trans('crud.delete_button') . '"></i></a>';
    }
