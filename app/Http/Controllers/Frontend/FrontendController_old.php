...
//                                ->withSubsectionSlug($section->slug)
        ...
        if (count($subsection) > 0) {
            ...
            if (count($courses) != 0) {
                ...
                //A paginação entra aqui, de forma separada.
                return view('frontend.courses.index')->withCourses($courses_query->paginate(20))->
                                withSections($this->sections->getAllSections())->withTags($tags)->withTitle($subsection->name)->
                                withColor($subsection->section->color)->withSlug($slug)->withActive($active_tag)->withSection($subsection->section);
            } else {
                ...
                return view('frontend.packages.index')->withPackages($epackages_query->paginate(20))->
                                withSections($this->sections->getAllSections())->withTitle($subsection->name)->withTags($tags)->
                                withColor($subsection->section->color)->withSlug($slug)->withActive($active_tag);
            }
        }
