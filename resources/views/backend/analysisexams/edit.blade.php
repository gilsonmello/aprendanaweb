...
@section ('title', trans('menus.analysisexam_management') . ' | ' . trans('menus.edit_analysisexam'))

@section('page-header')
    <h1>
        {{ trans('menus.analysisexams') }}
        <small>{{ trans('menus.edit_analysisexam') }}</small>
    </h1>
@endsection
...
@section ('breadcrumbs')
    ...
    <li class="active">{!! link_to_route('admin.analysisexams.create', trans('menus.edit_analysisexam')) !!}</li>
@stop

@section('content')

    {!! Form::model($analysisexam, ['route' => ['admin.analysisexams.update', $analysisexam->id], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH']) !!}

        <div class="form-group">
            {!! Form::label('nome', trans('validation.attributes.title'), ['class' => 'col-lg-2 control-label']) !!}
            <div class="col-lg-10">
                {!! Form::text('title', $analysisexam->title, ['class' => 'form-control', 'placeholder' => trans('strings.title')]) !!}
            </div>
        </div><!--form control-->
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <div class="col-lg-6">
            {!! Form::select("sources[]",  [''=>''] + $sources->lists("name","id")->all(), $analysisexam->source_id, ['class' => 'form-control select2' ])  !!}
    ...
    <div class="form-group">
        ...
        <div class="col-lg-6">
            {!! Form::select("institutions[]",  [''=>''] + $institutions->lists("name","id")->all(), $analysisexam->institution_id, ['class' => 'form-control select2' ])  !!}
    ...
    <div class="form-group">
        ...
        <div class="col-lg-6">
            {!! Form::select("offices[]",  [''=>''] + $offices->lists("name","id")->all(), $analysisexam->office_id, ['class' => 'form-control select2' ])  !!}
    ...
    <div class="pull-left">
            <a href="{{route('admin.analysisexams.index', ['page' => Session::get('lastpage', '1')])}}" class="btn btn-danger">{{ trans('strings.cancel_button') }}</a>
        </div>
