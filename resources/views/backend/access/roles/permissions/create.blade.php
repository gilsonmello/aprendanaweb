                ...
                @if (count($roles) > 0)
                    @foreach($roles as $role)
                        ...
                        <div class="create-permissions-switch">
                            <div class="onoffswitch">
                                <input type="checkbox" {{$role->id == 1 && Config::get('access.roles.Administrador_forced') ? "disabled='disabled' checked='checked'" : ''}} value="{{$role->id}}" name="permission_roles[]" class="toggleBtn onoffswitch-checkbox" id="role-{{$role->id}}">
