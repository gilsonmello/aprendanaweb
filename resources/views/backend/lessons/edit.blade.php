...
@section ('title', trans('menus.lesson_management') . ' | ' . trans('menus.edit_lesson'))

@section('page-header')
    <h1>
        {{ trans('menus.lessons') }}
        <small>{{ trans('menus.edit_lesson') }}</small>
    </h1>
@endsection
...
@section ('breadcrumbs')
    ...
    <li class="active">{!! link_to_route('admin.lessons.create', trans('menus.edit_lesson')) !!}</li>
@stop

@section('content')

    {!! Form::model($lesson, ['route' => ['admin.lessons.update', $lesson->id], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH', 'files' => true]) !!}
