...
@section ('title', trans('menus.tag_management') . ' | ' . trans('menus.edit_tag'))

@section('page-header')
    <h1>
        {{ trans('menus.tags') }}
        <small>{{ trans('menus.edit_tag') }}</small>
    </h1>
@endsection
...
@section ('breadcrumbs')
    ...
    <li class="active">{!! link_to_route('admin.tags.create', trans('menus.edit_tag')) !!}</li>
@stop

@section('content')

    {!! Form::model($tag, ['route' => ['admin.tags.update', $tag->id], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH']) !!}
