...
@section('page-header')
    <h1>
        {{ trans('menus.stats') }}
        <small>{{ trans('menus.stats_coursessales') }}</small>
    </h1>
@endsection

@section ('breadcrumbs')
    <li><a href="{!!route('backend.dashboard')!!}"><i class="fa fa-dashboard"></i> {{ trans('menus.dashboard') }}</a></li>
    <li class="active">{!! link_to_route('admin.stats.coursessales', trans('menus.stats_coursessales')) !!}</li>
@stop
...
@section('content')
    ...
    <table class="table table-striped table-bordered table-hover">
        ...
        <tr>
            ...
            <th width="20%" class="text-right">{{ trans('crud.courses.orders_count') }}</th>
        </tr>
        </thead>
        <tbody>
        @foreach ($courses as $course)
            <tr>
                <td>{!! $course->title !!}</td>
                <td class="text-right">{!! $course->orders_count !!}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
