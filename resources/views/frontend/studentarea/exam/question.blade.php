<div class="exam-explanation" >
    ...
    <div class="pull-left col-md-8 no-padding question-attributes">
        ...
        @if ( ($question_exec->question->subject != null) && ($question_exec->question->subject->parent != null) )
            @if($question_exec->execution->enrollment->exam != null && $question_exec->execution->enrollment->exam->result_level == 2)
            ...
            @else
                ...
                @if($question_exec->question->subject->parent->parent != null) <strong> [{{$question_exec->question->subject->parent->parent->name }}]  </strong>       @endif {{ $question_exec->question->subject->parent->name }}<br/>
    ...
    <div class="col-md-4 pull-right text-right no-padding" id="toolsDiv">
        ...
        <div class="btn-group" role="group" aria-label="..."><label class="fontesMarcadores">Marcadores <br>
                <button type="button" class="btn btn-default" data-toggle="tooltip" title="Marcador Rosa" onclick="javascript:activeHighlightRed();" id="highlightBtRed" style="background-color: #FD00FB; width: 35px;"><strong>&nbsp;</strong></button>
                <button type="button" class="btn btn-default" data-toggle="tooltip" title="Marcador Amarelo" onclick="javascript:activeHighlightYellow();" id="highlightBtYellow" style="background-color: #FEFF00;  width: 35px;"><strong>&nbsp;</strong></button>
                <button type="button" class="btn btn-default" data-toggle="tooltip" title="Marcador Azul" onclick="javascript:activeHighlightBlue();" id="highlightBtBlue" style="background-color: #00CFFF; width: 35px;"><strong>&nbsp;</strong></button></label>
        </div>

        <a class="mb-xs mt-xs mr-xs btn btn-default" data-toggle="tooltip" title="Limpar marcador" href="javascript:cleanHighlight();" id="highlightBtClean"><strong>Limpar</strong></a>
    </div>
        ...
        <div id="options" data-type="{{ $question_exec->question->answer_type }}">
            ...
            <table class="table mb-none">
                ...
                <tr><td>
                        ...
                        @if (($question_exec->question->image != null) && ($question_exec->question->image != null))
                            <br>
                            <img src="{{ imageurl("questions/", $question_exec->question->id, $question_exec->question->image) }}" class="img-responsive">
                        @endif
                        </td></tr>
                @if($letter = ord('a'))@endif
                @foreach($question_exec->question->answers as $answer)
                    <tr>
                        <td  @if($question_exec->grade !== null &&
                            ...
                            <div class="radio-custom radio-warning question_div" id="question_div_{{ $answer->id }}">
                                <input type="radio" value="{{$answer->id}}" id="checkbox{{ $answer->id }}" name="resposta"
                                       ...
                                       disabled
                                        @endif
                                        >
                                <label for="checkbox{{ $answer->id }}"><span id="{{ $answer->id }}" class="answer"
                                                                             @if($question_exec->grade !== null &&
                                        $question_exec->execution->simulation_mode == 0 && $question_exec->answersExecution->answers_chosen != $answer->id)
                                                                             style="color: lightgrey"
                                                                             @endif

                                                                             data-right="{{ ($answer->id * $question_exec->id) + $answer->is_right }}"><b>{!! chr($letter) !!}) </b>{!!  $answer->choice !!}</span><br/></label>
                            </div>
                        </td>
                    </tr>
                    @if($letter++)@endif
...
<div class="modal fade" id="questionContentModal" tabindex="-1" role="dialog" >
